...
Изменения vs оригинал:
  • merge()     — объединяет результаты нескольких парсеров (заполняет пустые поля)
  • Добавлен OpenRouterParser для цен и доп. метаданных
  • Префиксы hf: / lmsys: / or: работают как раньше
  • Без префикса: запускает HF + LMSYS + OR, объединяет результаты
"""
